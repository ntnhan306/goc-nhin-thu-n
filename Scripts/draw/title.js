...
function drawTitle(ctx, canvas) {
    ...
    const bannerWidth = titleImg.width * 0.4, bannerHeight = titleImg.height * 0.4;
    ...
    ctx.save();
    ctx.font = 'bold 34px Cambria Math';
    ctx.fillStyle = '#4a0e0e';
    ctx.textBaseline = 'middle';
    ctx.textAlign = 'center';
    ctx.shadowOffsetX = 1; ctx.shadowOffsetY = 1; ctx.shadowBlur = 2; ctx.shadowColor = '#3d2314';
    const radius = bannerWidth * 1.45, centerTextY = y + bannerHeight / 2 + 551;
    const arcSpan = Math.PI * 0.132, startAngle = Math.PI * -0.5 - arcSpan / 2;
    ...
    ctx.translate(-centerX-1.2, -textCenterY);
